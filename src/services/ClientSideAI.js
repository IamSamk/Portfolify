...
class ClientSideAI {
  ...
  async generateWebsite(elements, customInstructions = '') {
  ...
  async generateWithAI(elements, customInstructions) {
    const analysis = this.analyzeCanvas(elements);
    
    const prompt = `Create a modern website based on this design analysis:
${analysis}
...
Create HTML with embedded CSS.`;
  ...
  generateWithIntelligentRules(elements, customInstructions) {
    const analysis = this.analyzeCanvas(elements);
  ...
  analyzeCanvas(elements) {
    ...
    let analysis = `ELEMENTS ANALYSIS:
- ${textElements.length} text elements
- ${shapeElements.length} visual elements
- Color palette: ${colors.join(', ') || 'default scheme'}`;
    ...
    if (topElements.length > 0) analysis += `\n- Header section: ${topElements.length} elements`;
    if (middleElements.length > 0) analysis += `\n- Main content: ${middleElements.length} elements`;
    if (bottomElements.length > 0) analysis += `\n- Footer section: ${bottomElements.length} elements`;

    // Content analysis
    const allText = textElements.map(el => el.text || '').join(' ').toLowerCase();
    if (allText.includes('portfolio') || allText.includes('work')) {
      analysis += '\n- DETECTED: Portfolio/Creative website';
    } else if (allText.includes('contact') || allText.includes('business')) {
      analysis += '\n- DETECTED: Business/Professional website';
    } else if (allText.includes('blog') || allText.includes('article')) {
      analysis += '\n- DETECTED: Blog/Content website';
    }

    return analysis;
  }

  createAdvancedTemplate(elements, analysis, customInstructions) {
    const textElements = elements.filter(el => el.type === 'text');
    const colors = [...new Set(elements.map(el => el.fill || el.stroke).filter(Boolean))];
...
<body>
  ...
  enhanceAIOutput(aiHtml, elements, analysis) {
